-- =========================================================
-- Migration: Separate PAN and Citizenship No columns in clients table
-- =========================================================

-- 1. Add pan_no and citizenship_no columns if they don't exist
ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS pan_no TEXT,
  ADD COLUMN IF NOT EXISTS citizenship_no TEXT;

-- 2. Backfill: If pan_no is null and pan_or_citizenship exists, populate pan_no/citizenship_no
UPDATE public.clients
SET 
  pan_no = COALESCE(pan_no, CASE WHEN LENGTH(pan_or_citizenship) = 9 AND pan_or_citizenship ~ '^\d+$' THEN pan_or_citizenship ELSE NULL END),
  citizenship_no = COALESCE(citizenship_no, CASE WHEN NOT (LENGTH(pan_or_citizenship) = 9 AND pan_or_citizenship ~ '^\d+$') THEN pan_or_citizenship ELSE NULL END)
WHERE pan_or_citizenship IS NOT NULL AND (pan_no IS NULL AND citizenship_no IS NULL);

-- 3. Update bulk_insert_clients RPC to support separate pan_no, citizenship_no, pan_or_citizenship, and nid_number
