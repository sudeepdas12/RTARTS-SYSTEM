...
async function fixPvtLtd() {
  ...
  const pvtLtdCompanies = (clients || []).filter((c) => {
    ...
    const isMutualFund =
      /MUTUAL\s*FUND|\bMF\b|SAMRIDDHI|SAMUNNAT|PRAGATI|SAHABHAGITA|DHANABRIDDHI|SABAL|EQUITY\s*FUND|GROWTH\s*FUND|BALANCED\s*FUND|BLUECHIP|LARGE\s*CAP|FLEXI\s*CAP|FOCUS\s*(40|30)|SELECT\s*30/i.test(
        name,
      );
