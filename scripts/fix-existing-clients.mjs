...
async function run() {
  ...
  while (hasMore) {
    const { data, error } = await supabase
      .from("clients")
      .select(
        "id, boid, full_name, father_name, grandfather_name, payee_classification, holder_type",
      )
  ...
  console.log(
    `Found ${eligible.length} individual shareholders with family lineage currently marked as Tax Exempt.`,
  );

  for (const person of eligible) {
    console.log(
      ` -> BOID: ${person.boid} | Name: ${person.full_name} | Father: ${person.father_name || person.grandfather_name}`,
    );
  }
