...
async function check() {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .select(
        "payee_classification, payee_segment, gross_dividend, tax_amount, net_payable, client:clients(full_name, holder_type)",
      )
      .range(from, from + 999);
    if (error) {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .select(
        "payee_classification, payee_segment, gross_interest, tax_amount, net_interest, client:clients(full_name, holder_type)",
      )
      .range(from, from + 999);
    if (error) {
  ...
  const distinctTaxExemptNames = [
    ...new Set(taxExemptDivs.map((r) => r.client?.full_name).filter(Boolean)),
  ];
  ...
  console.log(
    "Individual humans wrongly under Tax Exempt (MUST BE 0):",
    invalidHumansInTaxExempt.length,
  );

  console.log("\n=== 4. VERIFYING KHIMADEVI LAGANI KOSH PVT.LTD ===");
  const khimadevi = (divRows || []).filter((r) =>
    (r.client?.full_name || "").includes("KHIMADEVI"),
  );
