...
function MutualFundPage() {
  ...
  const [calcTaxCategory, setCalcTaxCategory] = useState<
    "TAX_EXEMPTED" | "PUBLIC" | "INSTITUTION" | "CUSTOM"
  >("TAX_EXEMPTED");
  ...
  const { data: companies = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("companies")
        .select("id, company_code, company_name, company_type, sector_type")
        .order("company_name");
  ...
  // Contextual schemes/companies for Mutual Fund section (strictly shows Mutual Fund schemes only)
  const relevantCompanies = useMemo(() => {
    return companies.filter((c) => {
      ...
      if (type === "mutual fund" || sector === "mutual fund") return true;
      if (
  ...
  const { data: fiscalYears = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return Array.from(
        new Set((data || []).map((r: any) => r.fiscal_year).filter(Boolean)),
      ) as string[];
    },
    staleTime: 5 * 60 * 1000,
  });
  ...
  const {
    data: totals = { count: 0, paidCount: 0, pendingCount: 0, units: 0, gross: 0, tax: 0, net: 0 },
  } = useQuery({
    queryKey: [
      "mutual_fund_payables_totals",
      statusFilter,
      companyFilter,
      fyFilter,
      classFilter,
    ...
    queryFn: async () => {
      const data = await fetchAllRows<any>((from, to) => {
        let q = (supabase as any)
          .from("mutual_fund_payables")
          .select(
            "payment_status, shares_held, gross_dividend, tax_amount, net_payable, payee_classification, payee_segment, lot_name, created_at, payment_date",
          )
          .range(from, to);
  ...
  const upsert = useMutation({
    mutationFn: async () => {
      if (!canWrite)
        throw new Error(
          "Unauthorized: You do not have permission to create or edit mutual fund payables.",
        );
  ...
  const del = useMutation({
    mutationFn: async (id: string) => {
      if (!canDelete)
        throw new Error("Unauthorized: Only administrators can delete mutual fund payables.");
  ...
  const markPaid = useMutation({
    mutationFn: async () => {
      if (!canWrite)
        throw new Error("Unauthorized: You do not have permission to mark payables as paid.");
      if (!payOpen) return;
      const { error } = await (supabase as any)
        .from("mutual_fund_payables")
        .update({
  ...
  const handleImport = async (file: File) => {
    if (!canWrite) {
      toast.error("Unauthorized: You do not have permission to import mutual fund payables.");
      return;
    }
    try {
      type Row = {
              ...
              <>
                ...
                <Dialog open={open} onOpenChange={setOpen}>
                  ...
                  <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                      <DialogTitle>
                        {editing ? "Edit Mutual Fund Payable" : "New Mutual Fund Payable"}
                      </DialogTitle>
                    </DialogHeader>
                    {editing && (
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <div className="space-y-1.5 md:col-span-2">
                        ...
                        <Input
                          value={form.client_bank_name}
                          onChange={(e) => setForm({ ...form, client_bank_name: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5 md:col-span-2">
                        <Label>Bank Account No</Label>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => setCalcRateIsPerUnit(!calcRateIsPerUnit)}
                    title="Toggle % or Rs per unit"
                  >
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between border-b pb-1">
                    ...
                    <span className="font-medium">
                      {fmt(Number(calcUnits))} Units (Rs. {fmt(calcResult.totalPrincipal)})
                    </span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                  ...
                  <div className="flex justify-between border-b pb-1">
                    ...
                    <span className="font-medium">
                      ...
                      {calcTaxCategory === "TAX_EXEMPTED" && (
                        <Badge
                          variant="secondary"
                          className="ml-2 text-[10px] bg-emerald-100 text-emerald-700"
                        >
                          Tax Exempt
                        </Badge>
                      )}
          ...
          <div className="space-y-0.5">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              ...
              <Badge variant="outline" className="font-mono text-[11px] ml-1">
                {companyFilter === "all"
                  ? "All Schemes"
                  : companies.find((c) => c.id === companyFilter)?.company_code || "Selected"}{" "}
                {fyFilter !== "all" ? `— FY ${fyFilter}` : ""}
              </Badge>
            </CardTitle>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                SummaryReportService.exportMutualFundSummaryToExcel(
                  mfSummary.data ?? [],
                  `${companies.find((c) => c.id === companyFilter)?.company_code || "Mutual_Fund"}_Summary`,
                )
              }
              disabled={!mfSummary.data?.length}
            >
            ...
            <Button
              ...
              onClick={() =>
                SummaryReportService.exportMutualFundSummaryToPdf(
                  mfSummary.data ?? [],
                  companies.find((c) => c.id === companyFilter)?.company_name ||
                    "Mutual Fund Distribution",
                  `${companies.find((c) => c.id === companyFilter)?.company_code || "All Schemes"} — FY ${fyFilter !== "all" ? fyFilter : "All"}`,
                )
              }
              disabled={!mfSummary.data?.length}
            >
            ...
            <tbody className="divide-y divide-border font-mono">
              {mfSummary.isLoading ? (
                <tr>
                  <td
                    colSpan={8}
                    className="py-8 text-center text-muted-foreground font-sans text-xs"
                  >
                    Loading summary…
                  </td>
                </tr>
                ...
                <tr>
                  <td
                    ...
                    className="py-8 text-center text-muted-foreground font-sans text-xs"
                  >
                    No mutual fund payables for the selected scheme / fiscal year filter.
                  </td>
                </tr>
              ) : (
              ...
              <tfoot>
                <tr className="bg-muted/90 font-bold border-t-2 border-b-2 border-foreground/30 divide-x divide-border font-mono">
                  ...
                  <td className="py-2.5 px-3 text-right">
                    {fmt(mfSummary.data.reduce((a, r) => a + r.transaction_count, 0))}
                  </td>
                    ...
                    <TableRow key={payable.id} className="hover:bg-muted/30 transition-colors">
                      <TableCell>
                        <span className="font-mono text-xs font-semibold text-muted-foreground mr-1">
                          {payable.company?.company_code ?? "—"}
                        </span>
                        <span className="text-xs">{payable.company?.company_name}</span>
                      </TableCell>
                      <TableCell>
                        <div className="font-medium text-xs text-foreground">
                          {payable.client?.full_name ?? "—"}
                        </div>
                        {payable.client?.father_name && (
                          <div className="text-[10px] text-muted-foreground">
                            s/o {payable.client.father_name}
                          </div>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="text-right font-mono font-semibold text-primary">
                        {payable.shares_held || payable.after_bonus_kitta || payable.client?.kitta
                          ? (
                              payable.shares_held ||
                              payable.after_bonus_kitta ||
                              payable.client?.kitta
                            )?.toLocaleString()
                          : "—"}
                      </TableCell>
                      ...
                      <TableCell className="text-right font-mono">
                        {fmt(payable.gross_dividend)}
                      </TableCell>
                      <TableCell className="text-right font-mono text-amber-600">
                        {fmt(payable.tax_amount)}
                      </TableCell>
                      <TableCell className="text-right font-mono font-bold text-foreground">
                        {fmt(payable.net_payable)}
                      </TableCell>
                      <TableCell>
                        <div className="text-[11px] font-medium max-w-[140px] truncate">
                          {payable.bank_name || payable.client?.bank_name || "—"}
                        </div>
                          ...
                          variant={
                            payable.payment_status === "Paid"
                              ...
                              : payable.payment_status === "Partial"
                                ? "secondary"
                                : (payable as any).remarks?.includes("Rejected")
                                  ? "destructive"
                                  : "outline"
                          }
                        >
