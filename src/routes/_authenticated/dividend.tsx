...
function DividendPage() {
  ...
  const [calcTaxCategory, setCalcTaxCategory] = useState<
    "PUBLIC" | "INSTITUTION" | "TAX_EXEMPTED" | "CUSTOM"
  >("PUBLIC");
  ...
  useEffect(() => {
    if (settings?.dividend_tds_natural) {
      setCalcCustomTds((prev) => (prev ? prev : String(settings.dividend_tds_natural)));
    }
  }, [settings?.dividend_tds_natural]);

  const handleCalcDividend = () => {
    const shares = Number(calcShares);
    if (!shares || shares <= 0) {
      toast.error("Enter valid shares held");
      return;
    }
    if (
      (calcDivType === "Cash" || calcDivType === "Combined") &&
      (!calcCashRate || Number(calcCashRate) <= 0)
    ) {
      toast.error("Enter a valid cash dividend rate");
      return;
    }
    if (
      (calcDivType === "Bonus" || calcDivType === "Combined" || calcDivType === "Stock") &&
      (!calcBonusRatio || Number(calcBonusRatio) <= 0)
    ) {
      toast.error("Enter a valid bonus ratio (%)");
      return;
    }
  ...
  const { data: companies = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("companies")
        .select(
          "id, company_code, company_name, company_type, sector_type, dividend_rate, debenture_rate",
        )
        .order("company_name");
  ...
  // Contextual companies for Dividend section (strictly shows Equity companies & dividend issuers only)
  const relevantCompanies = useMemo(() => {
    return companies.filter((c) => {
      ...
      // Debentures / Bonds check
      if (type === "debenture" || sector === "debenture") return false;
      if (
        ...
        return false;
      if (c.debenture_rate && c.debenture_rate > 0) return false;

      // Mutual Fund check
      if (type === "mutual fund" || sector === "mutual fund") return false;
      if (
        name.includes("mutual fund") ||
        name.includes("yojana") ||
        name.includes("samriddhi") ||
        name.includes("samunnat") ||
        name.includes("scheme") ||
        name.includes("balanced fund") ||
        name.includes("growth fund") ||
        name.includes("equity fund") ||
        name.includes("pragati fund") ||
        code.includes("mf")
      ) {
        return false;
      }
  ...
  // Debounced search — 300ms before firing the server query
    ...
    queryFn: async () => {
      const data = await fetchAllRows<any>((from, to) => {
        let q = (supabase as any)
          .from("dividend_payables")
          .select(
            "payment_status, gross_dividend, tax_amount, bonus_tax, net_payable, shares_held, bonus_issued, payee_classification, payee_segment, lot_name, created_at, payment_date",
          )
          .range(from, to);
        ...
        if (classFilter !== "all") {
          if (classFilter === "PROMOTER") {
            ...
            q = q.or(
              "payee_classification.eq.TAX_EXEMPT,lot_name.ilike.%MUTUAL%,lot_name.ilike.%EXEMPT%",
            );
          } else if (classFilter === "INSTITUTION") {
            q = q.or(
              "payee_classification.eq.COMPANY_INSTITUTION,lot_name.ilike.%INSTITUT%,lot_name.ilike.%COMPANY%",
            );
          } else if (classFilter === "PUBLIC") {
            q = q.or(
              "payee_classification.eq.NATURAL_PERSON,payee_classification.eq.PUBLIC_LEGAL_PERSON,lot_name.ilike.%PUBLIC%",
            );
          }
        }
        return q;
  ...
  const fetchAllFiltered = useCallback(async (): Promise<Payable[]> => {
    return fetchAllRows<Payable>((from, to) => {
      let q = (supabase as any)
        .from("dividend_payables")
        .select(
          "*, client:clients(id, client_code, full_name, boid, father_name, grandfather_name, pan_or_citizenship, address, district, phone, bank_name, bank_account_no, holder_type, payee_classification, payee_segment), company:companies(id, company_code, company_name)",
        )
        .order("created_at", { ascending: false })
        .range(from, to);
      if (statusFilter !== "all") q = q.eq("payment_status", statusFilter);
    ...
    () =>
      AgmDividendSummaryReportService.generateReportFromPayables(
        ...
        selectedCompany?.company_name ||
          (companyFilter === "all" ? "All Companies" : "Selected Company"),
  ...
  const startNew = () => {
    setEditing(null);
    setForm({ ...emptyForm, fiscal_year: activeFyData || "" });
    setOpen(true);
  };
  ...
  const handleImport = async (file: File) => {
    try {
      ...
      rows.forEach((r, i) => {
        ...
        const gross =
          r.gross_dividend != null
            ? Number(r.gross_dividend)
            : shares != null && rate != null
              ? shares * rate
              : null;
        clean.push({
              ...
              <>
                ...
                <Dialog open={open} onOpenChange={setOpen}>
                  ...
                  <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                      <DialogTitle>
                        {editing ? "Edit Dividend Payable" : "New Dividend Payable"}
                      </DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) => setForm({ ...form, shares_held: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) => setForm({ ...form, dividend_rate: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          placeholder={String(previewGross || "")}
                          value={form.gross_dividend}
                          onChange={(e) => setForm({ ...form, gross_dividend: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Select
                          value={form.dividend_type}
                          onValueChange={(v) => setForm({ ...form, dividend_type: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={form.bonus_actual}
                          onChange={(e) => setForm({ ...form, bonus_actual: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={form.bonus_issued}
                          onChange={(e) => setForm({ ...form, bonus_issued: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          type="number"
                          step="0.0001"
                          value={form.bonus_fraction}
                          onChange={(e) => setForm({ ...form, bonus_fraction: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={form.after_bonus_kitta}
                          onChange={(e) => setForm({ ...form, after_bonus_kitta: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={form.bonus_tax}
                          onChange={(e) => setForm({ ...form, bonus_tax: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1.5">
          ...
          <CardContent className="pt-4 grid gap-4 lg:grid-cols-2">
            <div className="grid gap-4 sm:grid-cols-3 bg-card p-4 rounded-lg border shadow-sm">
              ...
              {(calcDivType === "Stock" || calcDivType === "Combined") && (
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="number"
                    step="0.01"
                    value={calcBonusRatio}
                    onChange={(e) => setCalcBonusRatio(e.target.value)}
                  />
                </div>
              )}
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    step="0.01"
                    value={calcCashRate}
                    onChange={(e) => setCalcCashRate(e.target.value)}
                  />
                  <Button
                    variant="outline"
                    size="icon"
                    className="shrink-0 h-9 w-9"
                    onClick={() => setCalcCashRateIsPerShare(!calcCashRateIsPerShare)}
                    title="Toggle % or Rs per share"
                  >
              ...
              <div className="sm:col-span-3 mt-2">
                <Button onClick={handleCalcDividend} className="w-full">
                  Calculate
                </Button>
              </div>
            </div>
                ...
                <div className="space-y-2 text-sm">
                    ...
                    <>
                      <div className="flex justify-between border-b pb-1">
                        <span className="text-muted-foreground">
                          Bonus Shares (Exact/Issued/Frac)
                        </span>
                        <span className="font-medium text-amber-600">
                          {fmtNr(calcResult.exactBonusShares)} /{" "}
                          {fmtNr(calcResult.issuedBonusShares)} /{" "}
                          {fmtNr(calcResult.fractionBonusShares)}
                        </span>
                      </div>
                      <div className="flex justify-between border-b pb-1">
                        <span className="text-muted-foreground">After Bonus Kitta</span>
                        <span className="font-medium text-blue-600">
                          {fmtNr(calcResult.afterBonusKitta)}
                        </span>
                      </div>
                    </>
                  )}
                  ...
                  <div className="flex justify-between border-b pb-1">
                    ...
                    <span className="font-medium">
                      {(calcResult.appliedTdsRate * 100).toFixed(2)}%
                    </span>
                  </div>
                  <div className="flex justify-between border-b pb-1">
                    <span className="text-muted-foreground">Cash TDS</span>
                    <span className="font-medium text-destructive">
                      {fmtNr(calcResult.cashTaxAmount)}
                    </span>
                  </div>
                  {calcResult.bonusTaxAmount > 0 && (
                    <div className="flex justify-between border-b pb-1">
                      ...
                      <span className="font-medium text-destructive">
                        {fmtNr(calcResult.bonusTaxAmount)}
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between pt-2">
                    ...
                    <span className="font-bold text-lg text-green-600">
                      Rs. {fmtNr(calcResult.netCashPayable)}
                    </span>
                  </div>
                </div>
              ) : (
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-xl font-bold font-mono tracking-tight mt-1">
                {(totals.totalShares ?? 0).toLocaleString()}
              </div>
              <div className="text-[11px] text-muted-foreground mt-0.5">
                {(totals.count ?? 0).toLocaleString()} records
              </div>
            </div>
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-xl font-bold font-mono tracking-tight mt-1 text-rose-600 dark:text-rose-400">
                NPR {fmt((totals.tax ?? 0) + (totals.bonusTax ?? 0))}
              </div>
        ...
        <Card className="border-border/60 shadow-sm hover:shadow transition-shadow">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-xl font-bold font-mono tracking-tight mt-1 text-amber-600">
                {(totals.bonusIssued ?? 0).toLocaleString()}{" "}
                <span className="text-xs font-normal">Kitta</span>
              </div>
        ...
        <CardHeader className="py-3 px-4 bg-muted/40 border-b flex flex-row items-center justify-between">
          <div className="space-y-0.5">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              ...
              <Badge variant="outline" className="font-mono text-[11px] ml-1">
                {agmSummaryReport?.companyCode || "All"}{" "}
                {agmSummaryReport?.fiscalYear ? `— FY ${agmSummaryReport.fiscalYear}` : ""}
              </Badge>
            </CardTitle>
            <p className="text-xs text-muted-foreground">
              Company-wise capital & payable breakdown by shareholder category (Promoter, Public,
              Local, etc.)
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
              ...
              <input
                ...
                placeholder={
                  agmSummaryReport?.detectedBonusRate
                    ? String(agmSummaryReport.detectedBonusRate)
                    : "0"
                }
              ...
              <input
                ...
                placeholder={
                  agmSummaryReport?.detectedDividendRate
                    ? String(agmSummaryReport.detectedDividendRate)
                    : "0"
                }
            ...
            <Button
              ...
              className="h-8 text-xs"
              onClick={() =>
                agmSummaryReport && AgmDividendSummaryReportService.exportToExcel(agmSummaryReport)
              }
            ...
            <Button
              ...
              className="h-8 text-xs"
              onClick={() =>
                agmSummaryReport && AgmDividendSummaryReportService.exportToPdf(agmSummaryReport)
              }
              disabled={!agmSummaryReport?.rows.length}
            >
          ...
          <CardContent className="p-0 overflow-x-auto">
            {summaryLoading ? (
              <div className="py-12 text-center text-muted-foreground text-sm flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Aggregating promoter, public, and local shareholding data
                </p>
              </div>
            ) : (
              <table className="w-full text-xs text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="bg-muted/80 text-foreground font-semibold border-b border-border divide-x divide-border">
                    <th className="py-2.5 px-3 text-center w-12 uppercase text-[11px] whitespace-nowrap">
                      S.N.
                    </th>
                  ...
                  {agmSummaryReport.rows.length === 0 ? (
                    ...
                    agmSummaryReport.rows.map((row) => (
                      ...
                      >
                        ...
                        <td className="py-2 px-3 text-right whitespace-nowrap">
                          {fmtNr(row.actualBonus)}
                        </td>
                        <td className="py-2 px-3 text-right whitespace-nowrap">
                          {fmtNr(row.issuedBonus)}
                        </td>
                        <td className="py-2 px-3 text-right whitespace-nowrap">
                          {fmtNr(row.remFraction)}
                        </td>
                ...
                {agmSummaryReport.rows.length > 0 && (
                  <tfoot>
                    <tr className="bg-muted/90 font-bold border-t-2 border-b-2 border-foreground/30 divide-x divide-border font-mono">
                      <td className="py-2.5 px-3 text-center whitespace-nowrap"></td>
                      <td className="py-2.5 px-3 font-sans uppercase whitespace-nowrap">TOTAL</td>
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.shareholderCount)}
                      </td>
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.kitta)}
                      </td>
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.actualBonus)}
                      </td>
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.issuedBonus)}
                      </td>
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.remFraction)}
                      </td>
                      ...
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.grossDividend)}
                      </td>
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.bonTax)}
                      </td>
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.divTax)}
                      </td>
                      <td className="py-2.5 px-3 text-right bg-emerald-100 text-emerald-950 dark:bg-emerald-900/60 dark:text-emerald-200 whitespace-nowrap">
                        {fmtNr(agmSummaryReport.total.netDividend)}
                      </td>
                      <td className="py-2.5 px-3 text-right font-sans whitespace-nowrap">
                        {(agmSummaryReport.total.composition ?? 0).toFixed(2)}%
                      </td>
                    </tr>
                  </tfoot>
                )}
                    ...
                    return (
                      <TableRow key={p.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <TableCell className="text-right font-mono font-semibold text-primary">
                          {p.shares_held || p.after_bonus_kitta || cl?.kitta
                            ? (p.shares_held || p.after_bonus_kitta || cl?.kitta)?.toLocaleString()
                            : "—"}
                        </TableCell>
                        <TableCell className="text-right font-mono text-xs">
                          {p.dividend_rate ? `${p.dividend_rate}%` : "—"}
                        </TableCell>
                        <TableCell className="text-right font-mono">
                          {fmt(p.gross_dividend)}
                        </TableCell>
