...
export function ReconciliationDashboard({ report }: ReconciliationDashboardProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Matched":
        return (
          <Badge className="bg-emerald-500/15 text-emerald-700 border-emerald-300">Matched</Badge>
        );
      case "Pledged":
        return (
          <Badge className="bg-amber-500/15 text-amber-700 border-amber-300">Pledged Shares</Badge>
        );
      case "Rejected":
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground capitalize">
                File Type: <span className="font-medium text-foreground">{report.fileType}</span> •{" "}
                {report.sourceType === "bank_statement"
                  ? `${report.grandTotal.totalRecords.toLocaleString()} record(s) analyzed`
                  : `${report.categories.length} sheet(s) analyzed`}
              </p>
            </div>
          </div>
          <div className="flex gap-6 text-right">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-primary">
                NPR{" "}
                {report.grandTotal.totalNetPayable.toLocaleString("en-IN", {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {report.grandTotal.totalRecords.toLocaleString()}
            </div>
          </CardContent>
        </Card>
        <Card
        ...
        >
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-xs text-emerald-700 uppercase font-medium">
              Matched / Success
            </CardTitle>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-emerald-600">
              {report.grandTotal.matchedRecords.toLocaleString()}
            </div>
          </CardContent>
        </Card>
        <Card
          className="cursor-pointer hover:border-amber-500 transition-colors"
          onClick={() => setSelectedStatusFilter("Pledged")}
        >
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-xs text-amber-700 uppercase font-medium">
              Pledged Accounts
            </CardTitle>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-amber-600">
              {report.grandTotal.pledgedCount.toLocaleString()}
            </div>
          </CardContent>
        </Card>
        <Card
        ...
        >
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-xs text-destructive uppercase font-medium">
              Rejected / Failed
            </CardTitle>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">
              {report.grandTotal.rejectedCount.toLocaleString()}
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {report.categories.map((cat) => (
                  <TableRow
                    ...
                    onClick={() =>
                      setSelectedCategory(
                        selectedCategory === cat.categoryName ? "ALL" : cat.categoryName,
                      )
                    }
                  >
                    <TableCell className="font-semibold">{cat.categoryName}</TableCell>
                    <TableCell className="text-right font-mono">
                      {cat.rowCount.toLocaleString()}
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {cat.totalKitta.toLocaleString()}
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {cat.totalGrossAmount > 0
                        ? cat.totalGrossAmount.toLocaleString("en-IN", { minimumFractionDigits: 2 })
                        : "-"}
                    </TableCell>
                    <TableCell className="text-right font-mono text-muted-foreground">
                      {cat.totalTaxAmount > 0
                        ? cat.totalTaxAmount.toLocaleString("en-IN", { minimumFractionDigits: 2 })
                        : "-"}
                    </TableCell>
                    <TableCell className="text-right font-mono font-bold text-primary">
                      {cat.totalNetPayable > 0
                        ? cat.totalNetPayable.toLocaleString("en-IN", { minimumFractionDigits: 2 })
                        : "-"}
                    </TableCell>
