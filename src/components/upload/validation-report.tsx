...
export function ValidationReport({ errors, fileName = "import" }: ValidationReportProps) {
  ...
  return (
    <Card
      className={
        isBlocking
          ? "border-destructive bg-destructive/5 mt-6"
          : "border-amber-400/50 bg-amber-500/5 mt-6"
      }
    >
      <CardHeader className="flex flex-row items-center justify-between py-4">
        <CardTitle
          className={`flex items-center gap-2 ${isBlocking ? "text-destructive" : "text-amber-600 dark:text-amber-400"}`}
        >
          {isBlocking ? (
            <>
              <AlertCircle className="h-5 w-5" />
              Validation Blocked — {hardErrors.length} Fatal Error{hardErrors.length > 1 ? "s" : ""}{" "}
              Found
            </>
          ) : (
            <>
              <AlertTriangle className="h-5 w-5 text-amber-600 dark:text-amber-400" />
              Validation Notice — {softErrors.length} Non-blocking Notice
              {softErrors.length > 1 ? "s" : ""} (Ready to Import)
            </>
          )}
        </CardTitle>
        <Button
          ...
          className={
            isBlocking
              ...
              : "text-amber-700 dark:text-amber-300 border-amber-400 hover:bg-amber-100 dark:hover:bg-amber-950/40"
          }
        ...
        <div className="max-h-[240px] overflow-y-auto rounded-md border border-muted bg-background text-sm">
          {errors.slice(0, 100).map((error, idx) => {
            ...
            return (
              <div
                key={idx}
                className="flex gap-4 p-2 border-b last:border-0 border-muted/50 hover:bg-muted/30"
              >
                <span className="font-medium min-w-[60px] text-muted-foreground">
                  Row {error.row || "File"}
                </span>
                <span className="font-semibold text-primary w-36 truncate capitalize">
                  {error.field}
                </span>
                <span
                  className={
                    isHard ? "text-destructive flex-1 font-medium" : "text-muted-foreground flex-1"
                  }
                >
                  {error.message}
                </span>
