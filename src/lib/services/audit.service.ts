...
import { recordLoginAttemptServerFn } from "@/lib/audit-logging.functions";
...
export const AuditService = {
  ...
  async recordLoginAttempt(opts: {
    ...
    try {
      ...
      await recordLoginAttemptServerFn({
        data: {
  ...
  formatAuditSummary(row: {
    ...
    if (
      ...
      const formattedAmount =
        amount !== undefined && amount !== null
          ? `NPR ${Number(amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
          : null;
      ...
      if (
        ...
        old.payment_status &&
        val.payment_status &&
        old.payment_status !== val.payment_status
      ) {
      ...
      return {
        ...
        subtitle:
          ...
          [
            ...
            shares,
            val.payment_reference ? `Ref: ${val.payment_reference}` : "",
          ]
            .filter(Boolean)
            .join(" · "),
        badge: status || typeLabel,
        badgeVariant:
          status === "Paid" ? "default" : status === "Pending" ? "outline" : "secondary",
      };
    }
    ...
    if (table === "clients") {
      ...
      const pan =
        val.pan_no || val.pan_or_citizenship
          ? `PAN/ID: ${val.pan_no || val.pan_or_citizenship}`
          : null;
    ...
    if (table === "reconciliation_results") {
      ...
      const expected =
        val.expected_amount !== undefined
          ? `Expected: NPR ${Number(val.expected_amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
          : "";
      const actual =
        val.actual_amount !== undefined
          ? `Actual: NPR ${Number(val.actual_amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
          : "";
      ...
      return {
        ...
        badge: result,
        badgeVariant:
          result === "Matched" ? "default" : result === "Rejected" ? "destructive" : "secondary",
      };
    }
    ...
    if (table === "companies") {
      return {
        ...
        subtitle: [
          val.company_code ? `Code: ${val.company_code}` : "",
          val.sector_type ? `Sector: ${val.sector_type}` : "",
    ...
    if (table === "payment_batches" || table === "payments") {
      const name =
        val.batch_name || (table === "payments" ? "Payment Transaction" : "Payment Batch");
      const total = val.total_amount
        ? `NPR ${Number(val.total_amount).toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
        : "";
    ...
    if (table === "upload_history") {
      return {
        ...
        subtitle: [
          val.status ? `Status: ${val.status}` : "",
          val.row_count ? `Rows: ${Number(val.row_count).toLocaleString()}` : "",
        ]
          .filter(Boolean)
          .join(" · "),
    ...
    return {
      ...
      subtitle:
        act === "DELETE"
          ? "Record was removed from database"
          : act === "INSERT"
            ? "New record inserted"
