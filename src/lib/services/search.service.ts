...
export const SearchService = {
  ...
  async globalSearch(query: string) {
    ...
    const [clientRes, companyRes] = await Promise.all([
      supabase
        .from("clients")
        .select(
          "id, full_name, boid, holder_type, payee_classification, pan_or_citizenship, bank_name, bank_account_no",
        )
        .or(
          `full_name.ilike.%${cleanQuery}%,boid.ilike.%${cleanQuery}%,pan_or_citizenship.ilike.%${cleanQuery}%,bank_account_no.ilike.%${cleanQuery}%`,
        )
        .limit(20),
      supabase
        .from("companies")
        ...
        .or(
          `company_name.ilike.%${cleanQuery}%,company_code.ilike.%${cleanQuery}%,isin.ilike.%${cleanQuery}%`,
        )
        .limit(10),
    ]);
  ...
  async searchInterestPayables(options: SearchPayablesOptions) {
    ...
    let q = (supabase as any)
      ...
      .select(
        "id, gross_interest, tax_amount, net_payable, net_interest, tds_rate, due_date, payment_date, payment_status, fiscal_year, instrument_ref, payee_classification, client:clients(id, full_name, boid, pan_or_citizenship, bank_name, bank_account_no), company:companies(id, company_code, company_name)",
      );
  ...
  async searchDividendPayables(options: SearchPayablesOptions) {
    ...
    let q = (supabase as any)
      ...
      .select(
        "id, gross_dividend, tax_amount, net_payable, tds_rate, payment_status, fiscal_year, dividend_type, payee_classification, client:clients(id, full_name, boid, pan_or_citizenship, bank_name, bank_account_no), company:companies(id, company_code, company_name)",
      );
