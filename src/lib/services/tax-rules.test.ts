...
const rules: TaxRule[] = [
  ...
  },
  {
    id: "3b",
    payable_category: "DIVIDEND",
    payee_classification: "FOREIGN_INVESTOR",
    tax_rate: 0.05,
    is_active: true,
  },
  {
    id: "4",
    payable_category: "INTEREST",
    payee_classification: "NATURAL_PERSON",
    tax_rate: 0.06,
    is_active: true,
  },
...
describe("centralized tax rules (payable type + investor category → rate)", () => {
  ...
  it("applies the spec rates (debenture 6/15, dividend 5/5, MF exempt)", () => {
    ...
    expect(getTaxRateFromRules(rules, "INTEREST", "FOREIGN_INVESTOR")).toBe(0.15);
    // Dividend
    ...
    expect(getTaxRateFromRules(rules, "DIVIDEND", "FOREIGN_INVESTOR")).toBe(0.05);
    // Mutual fund
...
describe("tax-exempt detection (isExemptFromTax)", () => {
  ...
  it("does not flag taxable classifications", () => {
    ...
    expect(isExemptFromTax("FOREIGN_INVESTOR")).toBe(false);
  });
