...
import {
  ...
  detectPayeeCategory,
  getPayeeTaxRate,
  getPayeeCategoryLabel,
} from "@/lib/services/payable-summary";
...
describe("End-to-End System Smoke Test", () => {
  ...
  describe("5. Heuristic Detection of Shareholder Categories", () => {
    it("detects corporate legal persons by name keywords", () => {
      expect(determineDebentureCategory({ client: { full_name: "SHIVAM CEMENTS LIMITED" } })).toBe(
        "INSTITUTION",
      );
      expect(
        determineDebentureCategory({
          client: { full_name: "SURYA JYOTI LIFE INSURANCE CO. LTD." },
        }),
      ).toBe("INSTITUTION");
      expect(determineDebentureCategory({ client: { full_name: "KUMARI BANK LIMITED" } })).toBe(
        "INSTITUTION",
      );
    });
    ...
    it("detects mutual funds and tax-exempt funds by name keywords", () => {
      ...
      expect(determineDebentureCategory({ client: { full_name: "RBB FOCUS 40" } })).toBe(
        "MUTUAL FUND",
      );
      expect(detectPayeeCategory({ full_name: "RBB Focus 40" })).toBe("MUTUAL_FUND");

      // All other unique SEBON mutual funds
      expect(determineDebentureCategory({ client: { full_name: "RBB MUTUAL FUND 1" } })).toBe(
        "MUTUAL FUND",
      );
      expect(determineDebentureCategory({ client: { full_name: "NIC ASIA SELECT 30" } })).toBe(
        "MUTUAL FUND",
      );
      expect(
        determineDebentureCategory({ client: { full_name: "CITIZENS SUPER 30 MUTUAL FUND" } }),
      ).toBe("MUTUAL FUND");
      expect(determineDebentureCategory({ client: { full_name: "NIBL SAMRIDDHI FUND 2" } })).toBe(
        "MUTUAL FUND",
      );
      expect(
        determineDebentureCategory({ client: { full_name: "GLOBAL IME SAMUNNAT SCHEME 1" } }),
      ).toBe("MUTUAL FUND");
      expect(determineDebentureCategory({ client: { full_name: "NIBL PRAGATI FUND" } })).toBe(
        "MUTUAL FUND",
      );
      expect(
        determineDebentureCategory({ client: { full_name: "KUMARI DHANABRIDDHI YOJANA" } }),
      ).toBe("MUTUAL FUND");
      expect(determineDebentureCategory({ client: { full_name: "SUNRISE BLUECHIP FUND" } })).toBe(
        "MUTUAL FUND",
      );
      expect(determineDebentureCategory({ client: { full_name: "NABIL FLEXI CAP FUND" } })).toBe(
        "MUTUAL FUND",
      );
      expect(determineDebentureCategory({ client: { full_name: "SANIMA LARGE CAP FUND" } })).toBe(
        "MUTUAL FUND",
      );
      expect(
        determineDebentureCategory({
          client: { full_name: "SIDDHARTHA SYSTEMATIC INVESTMENT SCHEME" },
        }),
      ).toBe("MUTUAL FUND");

      // Statutory Tax-Exempt / Retirement Funds
      expect(
        determineDebentureCategory({ client: { full_name: "CITIZEN INVESTMENT TRUST" } }),
      ).toBe("MUTUAL FUND");
      expect(
        determineDebentureCategory({ client: { full_name: "EMPLOYEES PROVIDENT FUND" } }),
      ).toBe("MUTUAL FUND");
      expect(determineDebentureCategory({ client: { full_name: "NAGARIK LAGANI KOSH" } })).toBe(
        "MUTUAL FUND",
      );
      expect(determineDebentureCategory({ client: { full_name: "SOCIAL SECURITY FUND" } })).toBe(
        "MUTUAL FUND",
      );
      expect(
        determineDebentureCategory({
          client: { full_name: "NEPAL BANK KARMACHARI AWAKASH KOSH BYAWASTHAPAN TRUST" },
        }),
      ).toBe("MUTUAL FUND");
    });

    it("detects natural persons as default public and avoids false positives on names containing kosh", () => {
      expect(determineDebentureCategory({ client: { full_name: "RAM BAHADUR SHRESTHA" } })).toBe(
        "PUBLIC",
      );
      expect(determineDebentureCategory({ client: { full_name: "SITA DEVI POUDEL" } })).toBe(
        "PUBLIC",
      );
      ...
      expect(
        determineDebentureCategory({
          client: { full_name: "Rikosh Giri", father_name: "Himalaya Giri" },
        }),
      ).toBe("PUBLIC");
      expect(detectPayeeCategory({ full_name: "Rikosh Giri", father_name: "Himalaya Giri" })).toBe(
        "PUBLIC",
      );

      expect(
        determineDebentureCategory({
          client: { full_name: "Hikosh Giri", father_name: "Himalaya Giri" },
        }),
      ).toBe("PUBLIC");
      expect(detectPayeeCategory({ full_name: "Hikosh Giri", father_name: "Himalaya Giri" })).toBe(
        "PUBLIC",
      );

      expect(
        determineDebentureCategory({
          client: { full_name: "KIKOSH THAPA", father_name: "SANJU SINGH THAPA" },
        }),
      ).toBe("PUBLIC");
      expect(
        detectPayeeCategory({ full_name: "KIKOSH THAPA", father_name: "SANJU SINGH THAPA" }),
      ).toBe("PUBLIC");

      expect(
        determineDebentureCategory({
          client: { full_name: "Kosh Nath Adhikari", father_name: "Mani Nath Adhikari" },
        }),
      ).toBe("PUBLIC");
      expect(
        detectPayeeCategory({ full_name: "Kosh Nath Adhikari", father_name: "Mani Nath Adhikari" }),
      ).toBe("PUBLIC");

      expect(
        determineDebentureCategory({
          client: { full_name: "KOSH RAJ POKHAREAL", father_name: "BHOJ RAJ POKHAREAL" },
        }),
      ).toBe("PUBLIC");
      expect(
        detectPayeeCategory({ full_name: "KOSH RAJ POKHAREAL", father_name: "BHOJ RAJ POKHAREAL" }),
      ).toBe("PUBLIC");

      expect(
        determineDebentureCategory({
          client: { full_name: "kosh raj subedi", father_name: "chandra Kanta Subedi" },
        }),
      ).toBe("PUBLIC");
      expect(
        detectPayeeCategory({ full_name: "kosh raj subedi", father_name: "chandra Kanta Subedi" }),
      ).toBe("PUBLIC");
    });

    it("classifies private limited companies with fund/kosh in name as INSTITUTION", () => {
      expect(
        determineDebentureCategory({ client: { full_name: "KHIMADEVI LAGANI KOSH PVT.LTD" } }),
      ).toBe("INSTITUTION");
      expect(detectPayeeCategory({ full_name: "KHIMADEVI LAGANI KOSH PVT.LTD" })).toBe(
        "INSTITUTION",
      );

      expect(
        determineDebentureCategory({ client: { full_name: "KOSH BYAWASTHAPAN COMPANY" } }),
      ).toBe("INSTITUTION");
      expect(detectPayeeCategory({ full_name: "KOSH BYAWASTHAPAN COMPANY" })).toBe("INSTITUTION");

      expect(determineDebentureCategory({ client: { full_name: "SHUBHA LAGANI PVT. LTD." } })).toBe(
        "INSTITUTION",
      );
