...
export interface ClassificationResult {
  ...
  rule_matched: string;
  confidence?: number;
  classification_status?: "AUTO_CLASSIFIED" | "REVIEW_REQUIRED" | "MANUALLY_OVERRIDDEN";
}
...
const MUTUAL_FUND_SCHEMES_REGEX =
...
export function smartClassify(input: ClassificationInput): ClassificationResult {
  ...
  const father = String(
    input.father_name ||
      ...
      input.FATHER_NAME ||
      input["FATHER'S NAME"] ||
      input.FATHER_NAME_MOTHER_NAME ||
      "",
  ).trim();
  const gfather = String(
    input.grandfather_name ||
      input.grandfatherName ||
      input.GRANDFATHER_NAME ||
      input["GRANDFATHER'S NAME"] ||
      input.GRANDFATHER_NAME_SPOUSE_NAME ||
      "",
  ).trim();
  ...
  const citizenship = String(
    input.citizenship || input.CITIZENSHIP || input.citizenship_no || input.CITIZENSHIP_NO || "",
  ).trim();
  const passport = String(input.passport_no || input.voter_id || "").trim();
  const dob = String(input.date_of_birth || input.dob || "").trim();
  const gender = String(input.gender || input.sex || "")
    .trim()
    .toUpperCase();
  const maritalStatus = String(input.marital_status || input.maritalStatus || "")
    .trim()
    .toUpperCase();
  const occupation = String(input.occupation || input.profession || "")
    .trim()
    .toUpperCase();
  ...
  const lotName = String(
    input.lot_name || input.sheetType || input.instrument_ref || "",
  ).toUpperCase();
  const dbClassification = String(input.payee_classification || "")
    .trim()
    .toUpperCase();
  const dbSegment = String(input.payee_segment || "")
    .trim()
    .toUpperCase();

  const isPromoterSegment =
    explicitType.includes("PROMOT") || lotName.includes("PROMOT") || dbSegment === "PROMOTER";
  const isLocalSegment =
    explicitType.includes("LOCAL") || lotName.includes("LOCAL") || dbSegment === "LOCAL";
  const isEmployeeSegment =
    explicitType.includes("STAFF") ||
    explicitType.includes("EMPLOYEE") ||
    lotName.includes("STAFF") ||
    lotName.includes("EMPLOYEE") ||
    dbSegment === "EMPLOYEE";
  ...
  if (dbClassification && dbClassification !== "UNCLASSIFIED" && !input.lot_name) {
    ...
    if (dbClassification === "NATURAL_PERSON" || dbClassification === "PUBLIC_LEGAL_PERSON") {
      const segment: PayeeSegment = isPromoterSegment
        ? "PROMOTER"
        : isLocalSegment
          ? "LOCAL"
          : isEmployeeSegment
            ? "EMPLOYEE"
            : "PUBLIC";
      const category: PayeeCategory = isPromoterSegment
        ? "PROMOTER"
        : isLocalSegment
          ? "LOCAL"
          : isEmployeeSegment
      ...
      return {
        payee_classification:
          dbClassification === "PUBLIC_LEGAL_PERSON" ? "PUBLIC_LEGAL_PERSON" : "NATURAL_PERSON",
  ...
  // =========================================================================
  // TIER 2: Corporate Registration Guard (PVT. LTD. / LIMITED / LTD / PARTNERS)
  ...
  // Must run BEFORE family lineage so contact person names don't misclassify companies.
  // =========================================================================
  ...
  // =========================================================================
  // TIER 3: True Human Family Lineage & Guardian Verification
  ...
  const hasMaritalStatus = Boolean(
    maritalStatus && (maritalStatus.includes("MARRIED") || maritalStatus.includes("SINGLE")),
  );
  const hasPersonalOccupation = Boolean(
    occupation &&
    /(STUDENT|HOUSEWIFE|SERVICE|TEACHER|DOCTOR|ENGINEER|AGRICULTURE|RETIRED|LAWYER|NURSE|PROFESSOR)/i.test(
      occupation,
    ),
  );
  ...
  if (isJointHumanHolder || NEPALI_SURNAMES_REGEX.test(nameUpper)) {
    ...
    return {
      ...
      tds_rate_dividend: 0.05,
      tds_rate_debenture: 0.06,
      rule_matched: isJointHumanHolder
        ? "Natural Person (Joint Human Account)"
        : "Natural Person (Nepali Name Structure)",
    };
  }
  ...
  return {
    payee_classification: "NATURAL_PERSON",
    payee_category: isPromoterSegment
      ? "PROMOTER"
      : isLocalSegment
        ? "LOCAL"
        : isEmployeeSegment
          ? "EMPLOYEE"
          : "PUBLIC",
    payee_segment: isPromoterSegment
      ? "PROMOTER"
      : isLocalSegment
        ? "LOCAL"
        : isEmployeeSegment
